package qa.auto.innotech.office;
...
public class Service {
    ...
    public static void checkDeleteDepartment(Department d) {
        removeDepartment(d);
        try (Connection con = DriverManager.getConnection("jdbc:h2:.\\Office")) {
            PreparedStatement depPs = con.prepareStatement("SELECT COUNT(ID) FROM Department WHERE NAME=?");
            depPs.setString(1, d.name);
            ResultSet depRs = depPs.executeQuery();
            assert (depRs.getInt(0) == 0);

            PreparedStatement empPs = con.prepareStatement("SELECT COUNT(ID) FROM Employee WHERE DepartmentID=?");
            empPs.setInt(1, d.departmentID);
            ResultSet employeesRs = empPs.executeQuery();

            assert (employeesRs.getInt(1) == 0);
        } catch (Throwable t) {
            t.printStackTrace();
        }
    }
